...
const ChangePasswordModal = ({open, onClose}) => {
    ...
    const handleSubmit = async(e) => {
        ...
        try {
           ...
           if(!data.message) throw new Error(data.error || "Failed")
        ...
        <div onClick={onClose} className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div onClick={(e) => e.stopPropagation()} className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md">
                <div className="flex items-center justify-between p-6 pb-0">
                    <h2 className="text-lg font-medium text-slate-900 flex items-center gap-2"><LockIcon className="h-5 w-5 text-slate-400"/>Change Password</h2>
